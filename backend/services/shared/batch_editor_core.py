# ─────────────────────────────────────────────────────────────
# backend/services/shared/batch_editor_core.py
# Editor em Massa — toda a lógica de processamento de vídeo (FFmpeg,
# fontes, overlay). Sem nada de FastAPI/router aqui de propósito — esse
# módulo é importado tanto pelo router (routers/batch_editor.py, só
# pros endpoints HTTP) quanto pela task do Celery
# (tasks/batch_editor_tasks.py, que roda o processamento de verdade no
# worker). Separar assim evita import circular entre os dois.
# ─────────────────────────────────────────────────────────────
...
async def process_one_video(job_id: str, index: int, video_url: str, req: BatchEditRequest) -> dict:
    """Processa 1 vídeo do lote de ponta a ponta (baixa, aplica filtro,
    overlay, sobe pro R2) e DEVOLVE o resultado — não mexe em nenhum
    estado global/compartilhado (isso é responsabilidade de quem chama:
    o router, no modelo antigo com BackgroundTasks, ou a task do Celery,
    que grava o resultado no Redis)."""
    out_path = None
    overlay_path = None
    try:
        ...
        return {"original_url": video_url, "final_url": final_url, "status": "done"}
    except Exception as e:
        logger.error(f"[batch_editor] job={job_id} idx={index} falhou: {e}")
        return {"original_url": video_url, "status": "error", "error": str(e)}
    finally:
